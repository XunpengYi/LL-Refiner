...
class UHD_ImageModel(BaseModel):
    """Base Deblur model for single image deblur."""
    def __init__(self, opt):
        super(UHD_ImageModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        from basicsr.models.losses import multi_VGGPerceptualLoss
        self.multi_cls = multi_VGGPerceptualLoss()
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            out_preds, preds_1, preds_2, coarse_preds = self.net_g(self.lq)

            if not isinstance(coarse_preds, list):
                coarse_preds = [coarse_preds]

            self.output = out_preds[-1]

            loss_dict = OrderedDict()

            # pixel loss
            l_pix = 0.
            l_pix += self.multi_cls(out_preds, preds_1, preds_2, self.gt)

            for coarse_pred in coarse_preds:
                gt_resized = F.interpolate(self.gt, size=coarse_pred.shape[2:], mode='bilinear', align_corners=False)
                l_pix = l_pix + 0.3 * self.cri_pix(coarse_pred, gt_resized)  # Pixel loss for low-res predictions
    ...
    def nonpad_test(self, img=None):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                # if the resolution is less than 4K, use the original inference
                if h < 4200 and w < 4200:
                    pred, _, _, _ = self.net_g_ema(img)
                else:
                    img_1 = img[:, :, :, 1::2]
                    img_2 = img[:, :, :, 0::2]
                    pred_1, _, _, _ = self.net_g_ema(img_1)
                    pred_2, _, _, _ = self.net_g_ema(img_2)
            ...
            with torch.no_grad():
                # if the resolution is less than 4K, use the original inference
                if h < 4200 and w < 4200:
                    pred, _, _, _ = self.net_g(img)
                else:
                    img_1 = img[:, :, :, 1::2]
                    img_2 = img[:, :, :, 0::2]
                    pred_1, _, _, _ = self.net_g(img_1)
                    pred_2, _, _, _ = self.net_g(img_2)
                    restored = torch.zeros_like(img)
                    restored[:, :, :, 1::2] = pred_1
                    restored[:, :, :, 0::2] = pred_2
                    pred = restored
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        current_metric = self.nondist_validation(dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        data_iter = tqdm(dataloader, desc=f'Evaluating rank:{rank}', unit='img')

        for idx, val_data in enumerate(data_iter):
        ...
        # average metric only on rank 0
        if with_metrics and rank == 0:
            self._log_validation_metric_values(current_iter, dataset_name, tb_logger)

        return current_metric if dist.get_rank() == 0 else 0.
